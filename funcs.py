...
def playMatchesBetweenVersions(env,
    ...
    if player1version == -1:
        ...
        player1_NN = ResCNN(config.REG_CONST, config.LEARNING_RATE,
                            env.input_shape, env.action_size,
                            config.HIDDEN_CNN_LAYERS)

        if player1version > 0:
            player1_network = player1_NN._get_model(env.name, run_version,
                                                    player1version)
    ...
    if player2version == -1:
        ...
        player2_NN = ResCNN(config.REG_CONST, config.LEARNING_RATE,
                            env.input_shape, env.action_size,
                            config.HIDDEN_CNN_LAYERS)

        if player2version > 0:
            player2_network = player2_NN._get_model(env.name, run_version,
                                                    player2version)
    ...
    for e in range(EPISODES):
        ...
        while done == 0:
            ...
            if turn < turns_until_tau0:
                action, pi, MCTS_value, NN_value = players[state.playerTurn][
                    'agent'].act(state, 1)
            else:
                action, pi, MCTS_value, NN_value = players[state.playerTurn][
                    'agent'].act(state, 0)
            ...
            for r in range(env.grid_shape[0]):
                logger.info([
                    ...
                    for x in pi[env.grid_shape[1] * r:(
            ...
            if done == 1:
                ...
                if value == 1:
                    ...
                    score = scores[players[state.playerTurn]['name']] + 1
                    scores[players[state.playerTurn]['name']] = score
