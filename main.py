...
while 1:
    ...
    if len(memory.ltmemory) >= config.MEMORY_SIZE:
        ...
        if iteration % 5 == 0:
            pickle.dump(memory,
                        open(run_folder + "memory/memory" + iteration + ".p",
                             "wb"))
        ...
        memory_samp = random.sample(memory.ltmemory,
        ...
        for s in memory_samp:
            ...
            lg.logger_memory.info('MCTS VALUE FOR %s: %f', s['playerTurn'],
                                  s['value'])
            lg.logger_memory.info('CUR PRED VALUE FOR %s: %f', s['playerTurn'],
                                  current_value)
            lg.logger_memory.info('BES PRED VALUE FOR %s: %f', s['playerTurn'],
                                  best_value)
